    ...
    <div class="mx-auto max-w-full px-2 sm:px-4 lg:px-6 xl:px-8">
      ...
      <div class="mb-4 sm:mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Proxy Attendance</h1>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
          ...
          <div class="sm:col-span-1 lg:col-span-5 flex flex-col sm:flex-row gap-2">
            <button
              @click="applyFilters"
      ...
      <!-- Results Count -->
      <div v-if="!loading && attendances.length > 0" class="mb-3 sm:mb-4">
      ...
      <!-- Empty State -->
      <div v-else-if="attendances.length === 0" class="rounded-lg bg-white p-6 sm:p-8 shadow-sm text-center">
        <svg class="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
        </svg>
        <h3 class="mt-3 text-base sm:text-lg font-semibold text-gray-900">No attendance records found</h3>
        <p class="mt-1 text-sm sm:text-base text-gray-600">
          {{ hasActiveFilters ? 'Try adjusting your filters' : 'No proxy attendance records to display' }}
        </p>
      </div>
          ...
          <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Hours Worked</th>
            ...
            <tbody class="divide-y divide-gray-200">
              ...
              >
                <td class="px-4 py-3">
                  <div>
                    <p class="text-sm font-semibold text-gray-900">{{ attendance.user?.name || 'Unknown' }}</p>
                    <p class="text-sm text-gray-500">{{ attendance.user?.current_job_information?.position_name || 'No Position' }}</p>
                  </div>
                </td>
                <td class="px-4 py-3">
                  <p class="text-sm text-gray-900">{{ attendance.user?.team?.name || 'No Team' }}</p>
                </td>
                <td class="px-4 py-3">
                  <p class="text-sm text-gray-900">{{ formatDate(attendance.attendance_date) }}</p>
                </td>
                <td class="px-4 py-3">
                  <p class="text-sm text-gray-900">{{ attendance.time_in || '-' }}</p>
                </td>
                <td class="px-4 py-3">
                  <p class="text-sm text-gray-900">{{ attendance.time_out || '-' }}</p>
                </td>
                <td class="px-4 py-3">
                  <p class="text-sm font-medium text-gray-900">{{ attendance.working_hours || '--:--' }}</p>
                </td>
                <td class="px-4 py-3">
                  <span :class="getStatusClass(attendance.status)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap">
                    {{ formatStatus(attendance.status) }}
                  </span>
                </td>
                <td class="px-4 py-3">
                  <p class="text-sm text-gray-900">{{ attendance.approver?.name || '-' }}</p>
                </td>
                <td class="px-4 py-3">
        ...
        <div class="lg:hidden divide-y divide-gray-200">
          ...
          >
            <div class="flex justify-between items-start mb-3">
              <div class="flex-1">
                <h3 class="text-base font-semibold text-gray-900">{{ attendance.user?.name || 'Unknown' }}</h3>
                <p class="text-sm text-gray-500">{{ attendance.user?.current_job_information?.position_name || 'No Position' }}</p>
                <p class="text-xs text-gray-400 mt-1">{{ attendance.user?.team?.name || 'No Team' }}</p>
              </div>
              <span :class="getStatusClass(attendance.status)" class="ml-2 px-2.5 py-1 text-xs font-medium rounded-full whitespace-nowrap">
                {{ formatStatus(attendance.status) }}
              </span>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
              ...
              <div>
                ...
                <p class="text-sm font-medium text-gray-900">{{ attendance.time_in || '-' }}</p>
              </div>
              <div>
                <p class="text-xs text-gray-500 mb-1">Time Out</p>
                <p class="text-sm font-medium text-gray-900">{{ attendance.time_out || '-' }}</p>
              </div>
              <div class="col-span-2">
                <p class="text-xs text-gray-500 mb-1">Approved By</p>
                <p class="text-sm font-medium text-gray-900">{{ attendance.approver?.name || '-' }}</p>
              </div>
            </div>

            <div v-if="attendance.notes" class="mb-3">
              <p class="text-xs text-gray-500 mb-1">Notes</p>
              <p class="text-sm text-gray-900 line-clamp-2">{{ attendance.notes }}</p>
            </div>
      ...
      <!-- Pagination -->
      <div v-if="attendances.length > 0" class="mt-4 sm:mt-6 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 class="text-xl font-bold text-gray-900">Edit Attendance Record</h2>
              <button 
                @click="closeModal"
            ...
            <form @submit.prevent="saveAttendance" class="p-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select 
                    v-model="currentAttendance.user_id"
                    required
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <input 
                    v-model="currentAttendance.attendance_date"
                    type="date"
                    required
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <input 
                    v-model="currentAttendance.time_in"
                    type="time"
                    required
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                ...
                <div>
                  ...
                  <input 
                    v-model="currentAttendance.time_out"
                    type="time"
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                </div>
                ...
                <div class="md:col-span-2">
                  ...
                  <textarea 
                    v-model="currentAttendance.notes"
                    rows="3"
                    class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ></textarea>
                </div>
      ...
      >
        <div class="flex min-h-screen items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <h3 class="text-lg font-semibold text-gray-900 text-center mb-2">Delete Attendance Record</h3>
            <p class="text-sm text-gray-600 text-center mb-6">
              Are you sure you want to delete this attendance record for 
              <span class="font-medium">{{ attendanceToDelete?.user?.name }}</span>? 
...
export default {
  ...
  data() {
    return {
      ...
      searchTimeout: null,
      // Cut-off period related
      cutoffPeriods: [],
      selectedCutoffPeriod: '', // Empty string = current period
  ...
  computed: {
    ...
    /**
     * First cut-off periods (10th - 24th)
     */
    firstCutoffPeriods() {
      return this.cutoffPeriods.filter(period => {
        // Check if the start date is the 10th of the month
    ...
    /**
     * Second cut-off periods (25th - 9th)
     */
    secondCutoffPeriods() {
      return this.cutoffPeriods.filter(period => {
        // Check if the start date is the 25th of the month
        const startDate = new Date(period.start_date)
  ...
  mounted() {
    ...
    // Auto-select current period after fetching
    this.$nextTick(() => {
  ...
  methods: {
    ...
    async fetchCutoffPeriods() {
      try {
        const response = await axios.get('/hr/proxy-attendance/cutoff-periods')
        this.cutoffPeriods = response.data.data
        
        // Auto-select the current period after loading
        this.$nextTick(() => {
          this.autoSelectCurrentPeriod()
        })
      } catch (error) {
    ...
    /**
     * Handle cut-off period selection
     */
    handleCutoffChange() {
      if (this.selectedCutoffPeriod === 'custom') {
        // Enable custom input but keep current values if any
        this.currentCutoffInfo = null
        return
      }

      // Use selected cut-off period
      ...
      if (period) {
        ...
        this.fetchAttendances(1)
      }
    },

    /**
     * Handle manual date changes in custom mode
     */
    onManualDateChange() {
      if (this.selectedCutoffPeriod === 'custom') {
    ...
    async fetchFormData() {
      try {
        const response = await axios.get('/hr/proxy-attendance/form-data')
        this.formData = response.data.data
        
        // Set current cut-off info from form data
    ...
    async fetchAttendances(page = 1) {
      ...
      try {
        ...
        const response = await axios.get('/hr/proxy-attendance', { params })
    ...
    editAttendance(attendance) {
      ...
      this.currentAttendance = {
        user_id: parseInt(attendance.user?.id),
        attendance_date: attendance.attendance_date || '',
        time_in: convertTo24Hour(attendance.time_in) || '',
        time_out: convertTo24Hour(attendance.time_out) || '',
    ...
    async saveAttendance() {
      this.saving = true
      try {
        await axios.put(`/hr/proxy-attendance/${this.editingAttendance.id}`, this.currentAttendance)
        this.showToast('Attendance record updated successfully', 'success')
        this.closeModal()
        this.fetchAttendances()
      } catch (error) {
        console.error('Error saving attendance:', error)
        const errorMessage = error.response?.data?.message || 'Failed to save attendance record'
    ...
    async deleteAttendance() {
      this.deleting = true
      try {
        await axios.delete(`/hr/proxy-attendance/${this.attendanceToDelete.id}`)
    ...
    async exportToCSV() {
      try {
        ...
        const response = await axios.get('/hr/proxy-attendance/export', { 
          params,
          responseType: 'blob'
        })
        ...
        // Extract filename from response or use default with cut-off info
        const contentDisposition = response.headers['content-disposition']
        let filename = `proxy_attendance_${new Date().toISOString().split('T')[0]}.csv`
